...
exports.getTitleFromType = getTitleFromType;
exports.getVCData = getVCData;
exports.getVCHTML = getVCHTML;
exports.guessCredentialTemplate = guessCredentialTemplate;
exports.humanizeCamelCase = humanizeCamelCase;
exports.objectToAttributesArray = objectToAttributesArray;
exports.renderVCHTML = renderVCHTML;
...
// THINK: should this be a peer dep or user supplied method to generate?
const typeToTemplateMap = {
  UniversityDegreeCredential: 'diploma',
  HackathonCredential: 'hackathon'
};

const cleanHTML = html => (0, _sanitizeHtml.default)(html, {
  allowedTags: _sanitizeHtml.default.defaults.allowedTags.concat(['style', 'img']),
...
function humanizeCamelCase(string) {
  let result;
...
function getTitle({
  ...
  let title = name || subjects[0] && subjects[0].title; // Get title from type of credential
...
function hashStr(str) {
  const sha = new _jssha.default('SHA-256', 'TEXT', {
    encoding: 'UTF8'
  });
  sha.update(str);
...
function getCredentialImage({
  ...
  const issuerImage = doDeepSearch(issuer, getLikelyImage) || generateImages && `data:image/png;base64,${new _identicon.default(hashStr(issuer.id || issuer), 128).toString()}` || null;
...
function extractHumanNameFields(s) {
  ...
  return s.name || (s.givenName ? s.familyName ? `${s.givenName} ${s.familyName}` : s.givenName : '') || s.assay || s.status || s.currentStatus || s.carrier || s.holder;
}
...
function getSubjectName({
  ...
  const subjects = Array.isArray(credentialSubject) ? credentialSubject : [credentialSubject];
  return subjects.map(s => s && mapDIDIfKnown(getObjectName(s, extractNameFields)), didMap).filter(s => typeof s === 'string').join(' & ');
}

function getSubjectDocuments({
  credentialSubject
}) {
  const subjects = Array.isArray(credentialSubject) ? credentialSubject : [credentialSubject];
...
function guessCredentialTemplate({
  type
}, customTemplateMap = {}) {
...
function objectToAttributesArray(object, result = [], parentName = '', parentProperty = '') {
  const keys = Object.keys(object);
...
async function renderVCHTML(data, options = {}) {
  ...
  const templateFn = customTemplates[templateId] || _templates.default[templateId];
...
async function getVCHTML(credential, options) {
